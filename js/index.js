...
function sugClick (e) {
  gifContainer.innerHTML = null
    ...
    .then((resp => {
        ...
        searchText.innerHTML = `<h1 class="searchtext">${searchTermi}</h1>`
...
lupa.addEventListener("click", deleteSearchText)

function deleteSearchText () {
  inputDeBusqueda.value = ""
}
  ...
  gifContainer.style.display = "flex"
	await fetch(
		...
		.then((resp => {
            ...
            let dataArray = resp.data
            deleteOldSearchs()
            searchText.innerHTML = `<h1 class="searchtext">${trendingTag}</h1>`
				displaySearchGif(dataArray);
			}
		))
		.catch((err) => console.log(err));
  ...
  const searchTerm = document.getElementById('search').value
	...
	await fetch(
		...
		.then((resp => {
            ...
            deleteOldSearchs()
            searchText.style.display = "block"
            searchText.innerHTML = `<h1 class="searchtext">${searchTerm}</h1>`
				displaySearchGif(dataArray);
			}
		))
		.catch((err) => console.log(err));
    ...
    if(dataArray.length === 0) {
        ...
        searchText.style.display = "block";
          searchText.innerHTML ="Busqueda no Encontrada";
               ...
               errorImg.classList.add("noresults")
